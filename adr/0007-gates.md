...
入口の型は自己申告でよい。bug-fix と偽って**依存構造**を変えれば出口ゲートが fail-closed で捕まえる。出口ゲートが捕まえるのは依存構造の乖離のみであり、依存構造を変えない意味論上の構造変更（既存モジュール内部での業務ルールの変更・追加等）は両ゲートの検出範囲外である — この残余はレビュー時の観点として扱う（ADR-0013 が明記する限界と同じ）。それでも入口に分類判定の賢さを持たせない: 分類判定を賢くしても検出範囲は広がらず、判断場面が増えるだけである。
