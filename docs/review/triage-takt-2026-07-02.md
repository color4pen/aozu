...
集計: 修正 14 / 解決済み 1 / 追跡 1 / 未決化 4（#3・#15・#20 → 論点 12、#9 → 論点 13）。

追記（同日）: 未決化 4 件のうち 3 件はその後 ADR-0018 で決着した — #15（addressed は計算導出）、#20（`request:` 行の廃止）、#9（plan は現在形の作業文書）。#3（designed への戻り）は方向のみ確定し、機構は loop 動詞の実装設計へ（論点 12 に縮約）。あわせて粒度改訂（旧論点 3）を ADR-0017、mark の並列意味論（旧論点 4）を ADR-0018-4 で決着。
