...
**窓の制約への戦略**（プロンプトの工夫では窓は広がらない。レビューにも同じスコープ規則を適用する）:

1. 繰り返す指摘クラスは決定的規則（C 規則・歯・conformance テスト）に卒業させる。LLM レビューは恒常の検証器ではなく規則の発見器
2. レビューの対象集合は参照グラフの近傍で機械的にシャーディングする（矛盾は参照で繋がった文書間に出る）
3. 現在形の正本（spec + open-questions）を小さく保つ。living docs / ADR 分離（ADR-0004）は窓対策でもあり、仕様が窓に収まらなくなったら設計の警報と扱う
4. レビューは網羅を主張しない。定型は「見ていない範囲」の明示を義務とする（findings ゼロと未走査の区別）。大域の品質・方向は人の領分として残る
