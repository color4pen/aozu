...
request 文書中の `[[id]]` は被覆の宣言と定義されており（integration.md §1・ADR-0012）、implemented 要素の引用は R2 で不合格になる。この設計は「設計 delta を経ずに実装済み要素へ触る」ことを防ぐ入口ゲートとして正しいが、**「この request が変更しない依存要素」を機械可読に宣言する場所が無い**（aosora findings #1。実装済みの ent を文脈として参照したい実需が確認済み）。現運用の回避はコード表記（参照抽出から除外）だが非自明で、依存情報が構造から消える。

設計候補:

1. request に「依存要素」の節（または行接頭辞）を定義し、`check --request` / coverage が節スコープで被覆と依存を区別する — 依存が機械可読になり、trace の材料にもなる。request 文書の構造への要求が増える（消費者非依存の原則 ADR-0012 との整合が論点）
2. 文脈引用の別記法を参照文法に足す（例: `[[~id]]`）— 文法の変更なので format-version に触れる可能性
3. 現運用（コード表記 = 抽出除外）を仕様に明文化するだけ — 最小コストだが依存は構造化されない

いずれも「引用 = 被覆」の単純さを崩すトレードオフがあり、trace 動詞（未実装）の設計と合わせて決めるのがよい。
