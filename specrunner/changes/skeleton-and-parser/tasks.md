...
- [ ] `package.json` を更新: `name: "aozu"`, `bin: { "aozu": "./src/cli/main.ts" }`, `dependencies: {}` (空オブジェクトを明示), `scripts: { "typecheck": "tsc --noEmit", "test": "bun test" }`
- [ ] `devDependencies` に `@types/bun` を追加（`bun add -d @types/bun`）
- [ ] `tsconfig.json` を作成: `target: ESNext`, `module: ESNext`, `moduleResolution: bundler`, `strict: true`, `noEmit: true`, `skipLibCheck: true`, `types: ["bun-types"]`, `include: ["src/**/*.ts"]`
- [ ] `src/cli/main.ts` を最小 stub として作成（`#!/usr/bin/env bun` + placeholder。CLI 結線は次 request）
- [ ] `.gitignore` に `dist/` が含まれることを確認（既存）
...
- [ ] `src/parse/types.ts` を作成し、以下の型を定義:
...
- [ ] `src/parse/index.ts` を作成し、型とパーサ関数を re-export
...
- [ ] `src/parse/id.ts` を作成
- [ ] ID 文法 `prefix "-" slug` (`slug = [a-z0-9]+("-"[a-z0-9]+)*`) の検証関数を実装
- [ ] 既知の prefix リスト（`mod`, `term`, `ent`, `inv`, `seq`, `top`, `plan`, `grp`, `adr`, `uc`, `scr`, `api`, `dat`, `flow`, `evt`, `ext`, `perm`, `dpl`）を定義
- [ ] ID からの prefix 抽出関数を実装
- [ ] `src/parse/id.test.ts` を作成:
...
- [ ] `src/parse/frontmatter.ts` を作成
- [ ] ファイル先頭の `---` 〜 `---` 間を flat `key: value` としてパース
- [ ] 値がカンマ区切りの場合は文字列配列に変換（例: `enabled: static, domain` → `["static", "domain"]`）
- [ ] flat でない行（インデントされた行、`key:` の後に改行して値が続く場合）を検出して `Diagnostic` を返す
- [ ] frontmatter が存在しないファイルは空の Record を返す
- [ ] `src/parse/frontmatter.test.ts` を作成:
...
- [ ] `src/parse/references.ts` を作成
- [ ] 行単位の参照抽出関数を実装: `[[<id>]]` パターンの全出現を抽出
- [ ] コードフェンス状態のトグル: `` ``` `` で始まる行で in/out を切り替え、フェンス内は参照抽出をスキップ
- [ ] インラインコード除外: 参照抽出前に行からバッククォート内テキストを除去（check.sh と同じ ``gsub(/`[^`]*`/, "")`` 相当）
- [ ] 各参照に file と line（1-based）を付与
- [ ] `src/parse/references.test.ts` を作成:
...
- [ ] `src/parse/declarations.ts` を作成
- [ ] 見出し要素の検出: `^#{2,3} (.+) \{#(<id>)\}$` にマッチする行から ID と表示名を抽出
- [ ] 文書要素の検出: frontmatter の `id:` フィールドから ID を抽出。表示名はファイル先頭の `# 見出し` から取得
- [ ] 各宣言に file・line・prefix を付与
- [ ] ID 文法検証（T-03）を適用し、違反は Diagnostic に追加
- [ ] `src/parse/declarations.test.ts` を作成:
...
- [ ] `src/parse/structured-lines.ts` を作成
- [ ] `責務:` 行の認識と値テキストの抽出
- [ ] `実装:` 行の認識とカンマ区切り値の分割
- [ ] 依存辺 `- [[a]] -> [[b]]` の認識と `DependencyEdge` の生成
- [ ] `## 登場要素` 配下の `- [[id]]` リストの認識
- [ ] `elements:` 行の認識（`[[id]]` のカンマ区切りリスト）
- [ ] `src/parse/structured-lines.test.ts` を作成:
  - `責務: コマンド解釈` の値抽出
...
- [ ] `src/parse/parser.ts` を作成
- [ ] 公開 API: `parseFiles(files: FileInput[]): ParseResult` を実装
- [ ] 各ファイルに対して frontmatter パース → 宣言抽出 → 参照抽出 → 構造化行認識を実行
- [ ] 結果を統合して `ParseResult` を返す
- [ ] `src/parse/parser.test.ts` を作成:
...
- [ ] `src/fs/reader.ts` を作成: 指定ディレクトリから `.md` ファイルを再帰的に列挙し、内容を読み込んで `FileInput[]` を返す関数
- [ ] `src/fs/index.ts` を作成し re-export
- [ ] Bun の `Bun.file` / `readdir` を使用
...
- [ ] `src/parse/integration.test.ts` を作成
- [ ] `src/fs/reader.ts` を使って `design/` の全 `.md` ファイルを読み込み、`parseFiles` に渡す
- [ ] 抽出された要素数が 25 であることを assert
- [ ] 抽出されたユニークな参照先 ID の種類が 15 であることを assert（`sort -u` 相当。check.sh が「参照 15 種」と呼ぶもの）
- [ ] `design/domain/invariants.md` のインラインコード内 `[[id]]` が参照に含まれないことを assert
- [ ] 25 要素の ID リストを明示的に assert（回帰検出用）:
...
- [ ] `design/static/dependencies.md` から抽出される `DependencyEdge` が 16 辺であることを assert
...
- [ ] `src/parse/diagnostics.test.ts` を作成
- [ ] 大文字を含む ID（例: `## Foo {#Mod-Parse}`）が位置つき diagnostic として報告されることを test
- [ ] 不正文字を含む ID（例: `## Foo {#mod_parse}`）が位置つき diagnostic として報告されることを test
- [ ] flat でない frontmatter（インデント行）が位置つき diagnostic として報告されることを test
- [ ] 正常な `design/` のパースで diagnostic が 0 件であることを test（design/ 自身は正しいはず）
...
- [ ] `src/package.test.ts` を作成
- [ ] `package.json` を読み込み、`dependencies` フィールドが `{}` または存在しないことを assert
...
- [ ] `bash tools/check.sh design` が引き続き `OK: 宣言 25 要素 / 参照 15 種` を返す（既存チェッカを壊していない）
