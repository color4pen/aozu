...
- [ ] `src/parse/request-citations.ts` を新規作成する
- [ ] 返り値の型 `RequestCitationResult` を定義する:
  ```
  ...
  ```
- [ ] `extractRequestCitations(content: string, filePath: string): RequestCitationResult` を実装する
- [ ] 行走査で以下を処理する:
...
- [ ] `stripInlineCode` と `extractRefsFromLine` は `./references.ts` から import して再利用する
- [ ] `src/parse/index.ts` に `extractRequestCitations` と `RequestCitationResult` のエクスポートを追加する
...
- [ ] `src/parse/request-citations.test.ts` を新規作成する
- [ ] 以下のテストケースを実装する:
...
- [ ] `bun test src/parse/request-citations.test.ts` が green
...
- [ ] `src/cli/commands/check.ts` に `extractRequestCitations` の import を追加する
- [ ] `handleCheckRequest` 内の `extractReferences` 呼び出しを `extractRequestCitations` に差し替える
- [ ] malformedLines の処理を追加: 各行に対して R3 error 診断を生成する
  ```
  {
    level: "error",
    code: "R3",
  ...
  ```
- [ ] R0 の判定を変更: `coverageRefs` から dedup した被覆引用の件数が 0 なら R0（従来は全引用で判定）
- [ ] R1 の判定を変更: 被覆引用の uniqueIds と dependencyIds の両方について、graph に存在しない ID は R1
- [ ] R2 の判定を変更: 被覆引用の uniqueIds のみ。implemented 要素は R2（dependencyIds は状態を問わない）
- [ ] `extractReferences` の import が不要になった場合は削除する（他に使用箇所がないことを確認）
...
- [ ] `src/cli/commands/check-request.test.ts` に以下のテストを追加する:
...
- [ ] テストフィクスチャに `createDesignFixture` の拡張が必要な場合は追加する（implemented 状態の要素が必要）
...
- [ ] `src/cli/commands/coverage.ts` に `extractRequestCitations` の import を追加する
- [ ] 草稿の引用抽出箇所（L213-215）を差し替える:
  ```typescript
  const draftContent = await Bun.file(draftPath).text();
  const citations = extractRequestCitations(draftContent, draftPath);
  ```
- [ ] malformedLines の処理を追加: 1 件でもあれば error 出力して return 1
  ```typescript
  ...
  ```
- [ ] `draftRefs` を `coverageRefs` から構築する（dependencyIds を含めない）:
  ```typescript
  const draftRefs = new Set(citations.coverageRefs.map((r) => r.targetId));
  ```
- [ ] `extractReferences` の import が不要になった場合は削除する
...
- [ ] `src/cli/commands/coverage.test.ts` に以下のテストを追加する:
  - 依存行にしか現れない要素が NOT_COVERED で exit 1（受け入れ基準 #7）
  - 草稿に不正な依存行があれば exit 1
  - 依存行と本文の両方に引用がある要素は被覆される（exit 0）
  - 依存行の無い既存テストが全て引き続き green（受け入れ基準 #8）
- [ ] `bun test src/cli/commands/coverage.test.ts` が green
...
- [ ] `bun test` が全 green（既存テスト含む）
- [ ] aozu 自身の `bun src/cli/main.ts check --dir design` の結果が不変（受け入れ基準 #9）
