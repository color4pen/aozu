...
- [ ] `handleInit(args: string[]): Promise<number>` を実装する:
...
- [ ] テンプレートの内容が spec/format.md §2, §3, §8 に適合することを手動確認する（`check` が exit 0 を返す構造であること）
...
- [ ] 空ディレクトリで `handleInit` を実行し、3 ファイルが生成されることを検証する
- [ ] 生成後に `handleCheck(["--dir", dir])` を呼び出し、exit 0 を返すことを検証する（init → check の e2e）
- [ ] 既存ディレクトリへの `handleInit` が exit 1 を返し、ファイルが変更されないことを検証する
- [ ] `--dir` フラグが正しくパースされることを検証する
- [ ] subprocess テスト: `Bun.spawn` で `bun src/cli/main.ts init --dir <path>` を実行し、stdout が空、stderr にメッセージが出力されることを検証する
...
- [ ] `enabled` の層名 → 型名のマッピングで型の有効性を判定する補助関数:
...
- [ ] `handleScaffold(args: string[]): Promise<number>` を実装する:
  - `--help` / `-h` で usage を stderr に出力して exit 0
...
- [ ] loop 有効 fixture を作成するヘルパー関数を定義する（manifest に `enabled: static, domain, dynamic, loop` を含む最小構造）
- [ ] `scaffold topic top-my-feature`: 生成されるファイルが `topics/my-feature.md` に配置され、frontmatter に `id: top-my-feature` と `status: open` を含むことを検証する
- [ ] `scaffold plan plan-my-plan`: 生成されるファイルが `plans/my-plan.md` に配置されることを検証する
- [ ] `scaffold seq seq-my-flow`: 生成されるファイルが `dynamic/my-flow.md` に配置され、`## 登場要素` セクションを含むことを検証する
- [ ] `scaffold adr adr-0001-my-decision`: 生成されるファイルが `adr/0001-my-decision.md` に配置されることを検証する
- [ ] ID 文法違反（`scaffold topic INVALID`）→ exit 1
- [ ] prefix 不一致（`scaffold topic seq-something`）→ exit 1
- [ ] 型無効（loop 無効 fixture で `scaffold topic top-xxx`）→ exit 1
- [ ] 既存 ID 衝突 → exit 1
- [ ] 見出し要素型（`scaffold mod mod-new`）→ exit 1 + 追記先案内メッセージ
- [ ] subprocess テスト: stdout/stderr 分離の検証
...
- [ ] フロンティア結果の型を定義する:
  ...
  ```
- [ ] `computeFrontier(graph, stateMap, manifest): Frontier` を実装する:
...
- [ ] frontmatter の `status` / `source` を取得するため、`parseFiles` の結果（frontmatters Map）を受け取るか、graph にアクセスする設計を決める。実装上は `parseFiles` の `frontmatters` を `computeFrontier` に渡すのが最も直接的
...
- [ ] `formatFrontier(frontier: Frontier): string` を実装する — フロンティア表示のフォーマット（stdout 用）
- [ ] `formatSummary(elementCount, refCount, checkOk): string` を実装する — 退化表示のフォーマット
- [ ] `handleStatus(args: string[]): Promise<number>` を実装する:
  - `--help` / `-h` で usage を stderr に出力して exit 0
  - `--dir <path>` フラグをパースする（デフォルト: `./design`）
...
- [ ] loop 有効 fixture を作成するヘルパー関数を定義する（open topic + designed 要素 + requested 要素を含む state.json 付き）
- [ ] `computeFrontier` の単体テスト:
...
- [ ] subprocess テスト: stdout に表示内容、stderr にはチェック診断のみ（あれば）が出力されることを検証
...
- [ ] `import { handleInit } from "./commands/init.ts"` を追加
- [ ] `import { handleScaffold } from "./commands/scaffold.ts"` を追加
- [ ] `import { handleStatus } from "./commands/status.ts"` を追加
- [ ] `register(registry, "init", handleInit, "initialize a design directory")` を追加
- [ ] `register(registry, "scaffold", handleScaffold, "create a new design document from template")` を追加
- [ ] `register(registry, "status", handleStatus, "show design frontiers and summary")` を追加
...
- [ ] `bun test` が全テスト green（既存 247 + 新規テスト）であることを確認する
...
- [ ] `src/cli/commands/init.ts`、`scaffold.ts`、`status.ts` からの import が `design/static/dependencies.md` の許可依存に違反しないことを確認する。3 ファイルはすべて `src/cli/` 配下であり `mod-cli` に属する。import 先:
