...
- [x] `src/check/attribution.ts` を新規作成する
- [x] `import type { Element } from "../parse/types.ts";` を使用する（`mod-check → mod-graph → mod-parse` の許可依存に従い、`src/graph/index.ts` 経由の re-export を使う: `import type { Element } from "../graph/index.ts";`）
- [x] `findOwningElement(elements: Element[], file: string, line: number): Element | undefined` を実装する:
...
- [x] `src/check/attribution.test.ts` を新規作成し、以下のケースを単体テストで固定する:
...
- [x] `src/check/rules/c03-ref-resolved.ts` を修正する
- [x] `KNOWN_PREFIXES` を `../../graph/index.ts` から import に追加する（既存の `extractPrefix` import と同じファイル）
- [x] `findOwningElement` を `../attribution.ts` から import する
- [x] 参照先の縮退スキップ条件を二段判定に変更する（現在: `!enabledPrefixes.has(targetPrefix)` → 変更後: `KNOWN_PREFIXES.has(targetPrefix) && !enabledPrefixes.has(targetPrefix)`）:
  ```typescript
...
- [x] 参照元の縮退スキップ条件も二段判定に変更する:
  ...
  ```
- [x] 未知 prefix の参照診断メッセージを識別可能にする（既知 prefix の未解決と区別するため）:
...
- [x] `src/check/rules/c03-ref-resolved.test.ts` に以下のテストケースを追加する:
...
- [x] `src/check/rules/c11-layer-direction.ts` を修正する
- [x] `findOwningElement` を `../attribution.ts` から import する
- [x] 参照元の所有要素特定を `findOwningElement` に切り替える:
  - 変更前: `graph.rawElements.find((el) => el.file === ref.file)`
  - 変更後: `findOwningElement(graph.rawElements, ref.file, ref.line)`
- [x] 変数名を `sourceElement` のままとし、`undefined` の場合の `continue` ガードは既存の `if (!sourceElement) continue;` をそのまま維持する
...
- [x] `src/check/rules/c11-layer-direction.test.ts` に以下のテストケースを追加する:
...
- [x] `tsc --noEmit` が成功する（型エラーゼロ）
- [x] `bun test` が全テスト green（既存 326 件 + 今回追加分 9 件 = 335 件）
- [x] `design/` に対する統合テスト（`src/check/integration.test.ts`）が引き続き違反ゼロを返す
- [x] `package.json` の `dependencies` が `{}` のまま（実行時依存ゼロを維持）
