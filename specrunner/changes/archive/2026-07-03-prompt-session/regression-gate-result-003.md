# Regression Gate Result — prompt-session — Iteration 3
...
Commit history inspected (HEAD `cabe81d` back to main):
...
The iter 3 code-fixer commit (`cabe81d`) contains **no source code changes** — only specrunner artifact files were touched. Consequently, none of the open findings from iteration 2 (F2, F3, F6, F7) were addressed. `bun test` and `tsc --noEmit` remain green (no regressions in passing tests).
...
All four stage-gate tests (loop 無効 / topic 不存在 / design 不在 / 引数不足) include `expect(stdout).toBe("")`. Fixed in iter 1 (`f75616b`); still present in current HEAD (`cabe81d`). No regression.
...
`createSessionFixture` generates `static/modules.md` containing `実装: src/core/` and `実装: src/cli/`. No assertion of the form `expect(stdout).not.toContain("実装:")` exists anywhere in the test file. The only `.not.toContain()` assertion in the session tests is `expect(stdout).not.toContain("UNIQUE_SCOPE_BOUNDARY_MARKER")` (3-hop boundary). The iter 3 code-fixer made no code changes. Finding carried over from iter 2 unchanged.
...
No test passes a non-"top" prefix ID (e.g., `ent-order`) to `--topic session`. The implementation check at `prompt.ts:389` (`topicEl.prefix !== "top"`) is correct but untested. The four existing stage-gate tests use `top-my-topic` (valid), `top-nonexistent` (absent — hits the `!topicEl` branch, not the prefix branch), missing `--dir`, or missing `--topic`. The iter 3 code-fixer made no code changes. Finding carried over from iter 2 unchanged.
...
`prompt.ts:450` still outputs `(no 責務: line found)` while design.md D5 says "見出しのみを出力する". `review-feedback-001` explicitly set Fix=**no** ("設計判断が必要なため fixer でスキップしても可"). Consistent with iter 1 and iter 2 gate decisions. Not treated as a regression.
...
design.md Risks/Trade-offs states "要約に spec/format.md のバージョン注記を含めることで、手動更新のトリガを可視化する". The `FORMAT_RULES_SUMMARY` constant in `session.ts` (lines 36–87) contains no `format-version: 0` reference or link to `spec/format.md`. The iter 3 code-fixer made no code changes. Finding carried over from iter 2 unchanged.
...
`docs/open-questions.md §8` says "manifest と形式規則の要約は常に注入" but `SessionInput §6` injects only the `enabled` list (omitting `request-template`, `request-output-dir`, etc.). The rationale for this narrowing interpretation ("D8: session は消費者設定に依存しない") exists in design.md but is not cross-referenced in §8. The "初版実装済み" note added in `b212de6` records hop-rule constants but does not address this gap. The iter 3 code-fixer made no code changes to `docs/open-questions.md`. Finding carried over from iter 2 unchanged.
