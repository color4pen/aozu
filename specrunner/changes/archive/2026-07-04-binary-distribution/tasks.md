...
- [x] Create `src/cli/version.ts` that exports an async `getVersion(): Promise<string>` function
- [x] The function checks `globalThis.__AOZU_VERSION` first (compile-time injected constant)
- [x] If `globalThis.__AOZU_VERSION` is a non-empty string, return it directly
- [x] Otherwise, fall back to reading `package.json` version via `Bun.file()` (current logic from `main.ts`)
- [x] Add a TypeScript declaration for `globalThis.__AOZU_VERSION` (e.g., `declare var __AOZU_VERSION: string | undefined;` in the module or a `.d.ts` file) so `tsc --noEmit` passes
- [x] Refactor `src/cli/main.ts` lines 41-47: replace the inline `package.json` reading with a call to `getVersion()` from the new module
...
- [x] Create `tests/binary.test.ts` with tests that:
...
- [x] Clean up the temporary binary in `afterAll`
- [x] Ensure the test file is excluded from `npm pack` (already covered by existing `files` field in `package.json` which only includes `src`, `README.md`, `LICENSE`)
...
- [x] Edit `.github/workflows/publish.yml`:
...
- [x] The `build-binaries` job steps:
...
- [x] Add a final `upload-binaries` job that:
...
- [x] Add tests to `tests/packaging.test.ts` (or a new `tests/binary-distribution.test.ts`) that grep `.github/workflows/publish.yml` for:
...
- [x] Create `install.sh` at the repository root
- [x] The script must start with `#!/bin/bash` and `set -euo pipefail`
- [x] Detect OS: `uname -s` mapped to `darwin` or `linux`; abort with error message for unsupported OS
- [x] Detect architecture: `uname -m` mapped to `arm64` (for `arm64`/`aarch64`) or `x64` (for `x86_64`/`amd64`); abort with error message for unsupported architecture
- [x] Construct the binary name: `aozu-${os}-${arch}`
- [x] Fetch the latest release tag from GitHub API: `https://api.github.com/repos/color4pen/aozu/releases/latest`
- [x] Download the binary: `https://github.com/color4pen/aozu/releases/download/${tag}/${binary_name}`
- [x] Download `SHA256SUMS` from the same release and verify the downloaded binary's SHA-256 hash (`sha256sum -c` on linux / `shasum -a 256 -c` on darwin, filtered to the target binary); abort with an error message on mismatch
- [x] Create `~/.local/bin` if it does not exist
- [x] Place the binary at `~/.local/bin/aozu` and `chmod +x` it
- [x] Run `~/.local/bin/aozu --version` and print the output as a connectivity check
- [x] If `~/.local/bin` is not in `$PATH`, print a guidance message suggesting the user add it
...
- [x] Add a test (in `tests/binary-distribution.test.ts` or `tests/packaging.test.ts`) that:
...
- [x] Edit `.github/workflows/ci.yml`: add a `binary-smoke` job that:
...
- [x] This runs on every PR and push to main (existing CI trigger)
...
- [x] Add a "Single Binary" subsection to the "## 導入" section of `README.md`
- [x] Include the one-liner: `curl -fsSL https://raw.githubusercontent.com/color4pen/aozu/main/install.sh | bash`
- [x] Briefly explain the two-channel distribution (npm for JS ecosystem, binary for language-agnostic use)
- [x] Link to the GitHub Releases page for manual download
