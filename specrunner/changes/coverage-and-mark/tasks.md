...
- [ ] src/state/writer.ts を新規作成する
- [ ] `writeDesignState(designDir: string, stateMap: StateMap): Promise<void>` を実装する:
...
- [ ] `CoverageDiagnostic` 型を定義する: `{ level: "error" | "warning"; code: string; elementId: string; message: string }`
- [ ] `CoverageResult` 型を定義する: `{ pass: boolean; errors: CoverageDiagnostic[]; warnings: CoverageDiagnostic[] }`
- [ ] `GroupGraph` 型を定義する: `{ groupElements: Map<string, Set<string>>; afterEdges: Set<string> }`
- [ ] `verifyCoverage(groupElementIds: string[], draftRefs: Set<string>, graph: Graph, stateMap: StateMap, groupGraph: GroupGraph): CoverageResult` を実装する:
...
- [ ] src/plan/coverage.test.ts を新規作成し、以下のテストを実装する:
...
- [ ] `handleCoverage(args: string[]): Promise<number>` を実装する:
...
- [ ] fixture ヘルパーの作成: loop 有効の manifest・plan ファイル（1 グループ）・対象要素の設計文書・state.json を生成する関数
- [ ] 正常系テスト（被覆完全）:
...
- [ ] 部分適用状態を作らない: stateMap のコピーを変更してから一括書き込みする
- [ ] stdout には何も書かない
...
- [ ] fixture ヘルパーの作成: loop 有効の manifest・requested 状態の state.json を生成する関数
- [ ] 正常遷移テスト:
...
- [ ] src/cli/commands/status.ts の wrapper `computeFrontier` を更新する:
...
- [ ] status.test.ts の openTopics 関連テストを更新する:
...
- [ ] `topicTemplate` 関数から `status: open` 行を除去する:
  - 変更前: `---\nid: ${id}\nstatus: open\n---`
  - 変更後: `---\nid: ${id}\n---`
- [ ] scaffold.test.ts の topic テンプレート関連テストを更新する:
...
- [ ] `import { handleCoverage } from "./commands/coverage.ts"` を追加
- [ ] `import { handleMark } from "./commands/mark.ts"` を追加
- [ ] `register(registry, "coverage", handleCoverage, "verify draft coverage and transition to requested")` を追加
- [ ] `register(registry, "mark", handleMark, "transition element states")` を追加
...
- [ ] design/static/modules.md の mod-state 責務行が「読み書き」を反映しているか確認する（現在の「読み書きと状態遷移」は既に正確。変更不要なら skip）
...
- [ ] 既存テストは、意味論変更の対象（openTopics 系・scaffold topic 系）の更新を除き無変更で green
- [ ] architecture.test.ts が green であることを確認する
