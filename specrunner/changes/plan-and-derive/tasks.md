...
- [ ] src/plan/frontier.ts を新規作成し、`computeFrontier` / `Frontier` / `IMPLEMENTATION_PREFIXES` を移動する
- [ ] `computeFrontier` の import 文を調整する: `Graph` は `../graph/types.ts` から、`StateMap` は `../state/types.ts` から import する。`getEnabledPrefixes` は `../check/manifest.ts` から import するが、**mod-plan -> mod-check は不許可**のため、`getEnabledPrefixes` の呼び出しを caller 側（mod-cli）で行い、enabledPrefixes: Set<string> を引数として受け取る形にリファクタリングする
- [ ] `computeFrontier` のシグネチャを `computeFrontier(graph, stateMap, enabledPrefixes, frontmatters)` に変更する（manifest の代わりに enabledPrefixes を受け取る）
- [ ] `ParseResult["frontmatters"]` 型は mod-parse 由来。mod-plan -> mod-parse は不許可のため、frontmatters の型を `Map<string, Record<string, string | string[]>>` としてインラインで記述するか、mod-graph の re-export（`ParseResult` は graph/index.ts で re-export 済み）経由で取得する（mod-plan -> mod-graph は許可済み）
- [ ] src/cli/commands/status.ts の import を src/plan/frontier.ts に差し替え、`getEnabledPrefixes(manifest)` を呼び出して結果を `computeFrontier` に渡すよう修正する
- [ ] src/plan/frontier.ts から Frontier 型と computeFrontier を export する
- [ ] 既存の status.test.ts が全て green であることを確認する
...
- [ ] src/graph/attribution.ts を新規作成し、`findOwningElement` を移動する
- [ ] src/check/attribution.ts を src/graph/attribution.ts からの re-export に書き換える（`export { findOwningElement } from "../graph/attribution.ts"`）
- [ ] src/graph/index.ts に `findOwningElement` の re-export を追加する
- [ ] 既存の attribution.test.ts が全て green であることを確認する
...
- [ ] `extractElementBody(elementId: string, graph: Graph, files: FileInput[]): string | null` を実装する
...
- [ ] `extractAllBodies(ids: string[], graph: Graph, files: FileInput[]): Map<string, string>` を実装する（複数要素の本文を一括取得する便利関数）
- [ ] body.ts のテストを作成する（src/graph/body.test.ts）:
...
- [ ] `computeNeighborhood(seedIds: string[], graph: Graph, maxHops: number): Set<string>` を実装する
...
- [ ] neighborhood.ts のテストを作成する（src/graph/neighborhood.test.ts）:
  - 1 hop 近傍が正しく返るケース
  - 2 hop 近傍が正しく返るケース
  - seedIds を含まないことの検証
...
- [ ] stdout には何も書かないことを確認する（成果物はファイル。成功メッセージは stderr）
...
- [ ] loop 有効・designed 要素ありの fixture を作成するヘルパー関数を定義する（要素間参照・requested 要素を含む）
- [ ] 正常系テスト:
...
- [ ] subprocess テスト: stdout が空、stderr にメッセージ
...
- [ ] 入力型を定義する:
  ...
  ```
- [ ] `buildDeriveInstruction(input: DeriveInput): string` を実装する:
  - 指示テキストを組み立てる:
    ...
    - 出力先: outputDir のパス
- [ ] derive.ts のテストを作成する（src/prompt/derive.test.ts）:
...
- [ ] `import { handlePlan } from "./commands/plan.ts"` を追加
- [ ] `import { handlePrompt } from "./commands/prompt.ts"` を追加
- [ ] `register(registry, "plan", handlePlan, "generate a plan from designed elements")` を追加
- [ ] `register(registry, "prompt", handlePrompt, "generate prompts for design workflows")` を追加
...
- [ ] spec/format.md §3 の manifest セクションに以下を追補する:
...
- [ ] spec/integration.md §4 の「設定で注入され」の箇所に注入点を明記する:
  - 「manifest frontmatter の `request-template` と `request-output-dir` で注入される」
- [ ] design/static/modules.md の mod-plan 責務行を更新する:
  - 旧: `責務: plan の生成・coverage 検証・グループへの request 記録。`
  - 新: `責務: plan の生成・coverage 検証。`
- [ ] `bun src/cli/main.ts export rules --out design/rules.json` を実行して rules.json を更新する
...
- [ ] `bun test` が全テスト green（既存 336 + 新規テスト）であることを確認する
- [ ] `bun src/cli/main.ts check` が exit 0 であることを確認する（本リポジトリの design/ に対して）
- [ ] `bun src/cli/main.ts export rules --verify` が exit 0 であることを確認する
- [ ] `package.json` の `dependencies` が空のままであることを確認する
...
- [ ] architecture.test.ts（歯テスト）が green であることを確認する
