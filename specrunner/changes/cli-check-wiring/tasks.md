...
- [x] `src/cli/registry.ts` を作成し、以下を実装:
...
- [x] `src/cli/registry.test.ts` を作成:
...
- [x] `src/cli/format.ts` を作成し、以下を実装:
...
- [x] `src/cli/commands/check.ts` を作成し、`handleCheck(args: string[]): Promise<number>` を実装:
...
- [x] `--help` フラグがあれば check コマンドの使い方を stderr に出力し 0 を返す
...
- [x] `src/cli/commands/check.ts` 内に `handleCheckRequest(args: string[], designDir: string): Promise<number>` を実装（T-04 の handler から呼ばれる）:
...
- [x] スタブの `console.log("aozu — design layer CLI (not yet wired)")` を削除
...
- [x] `src/cli/commands/check.test.ts` を作成
- [x] テスト: 本リポジトリ自身の `design/` で `handleCheck` が exit 0 を返す（`tools/check.sh` と同判定）
  - `handleCheck([])` または `handleCheck(["--dir", "<repo>/design"])` で呼び出す
  - 戻り値が 0 であることを assert
- [x] テスト: 違反を含む fixture で exit 1 を返す
...
- [x] テスト: design ディレクトリ不在で exit 2 を返す
  - `handleCheck(["--dir", "/tmp/nonexistent-" + Date.now()])` が 2 を返すことを assert
- [x] テスト: stdout に診断が混ざらないことの確認
...
- [x] `tsc --noEmit` が成功する
- [x] `bun test` が全テスト green（既存 184 テスト + 新規テストすべて）
- [x] `package.json` の `dependencies` が `{}` のまま
- [x] `bun src/cli/main.ts check` が exit 0 を返すことを手動確認
- [x] `bun src/cli/main.ts check --request <path>` が機能することを手動確認（実在 ID の request 文書で）
- [x] `tools/check.sh design` が引き続き動作する（共存確認）
